...
void Bresenham::LimpiarCeldas()
{
	...
	while (this->auxA->sig != NULL)
	{
		...
		if (this->estadoBresenham == 2 && contador < 2)
			SetWindowText(this->auxA->celda, "");
		this->auxA = this->auxA->sig;
		contador++;
	}
	if ((this->estadoBresenham == 0) || this->estadoBresenham == 1)
		SetWindowText(this->auxA->celda, "");
}
...
void Bresenham::LimpiarResultado()
{
	...
	if (this->estadoBresenham == 0)
		...
		EnableWindow(this->btnPuntos[2], false);
	else if (this->estadoBresenham == 2)
		EnableWindow(this->btnPuntos[3], false);
}
...
void Bresenham::Circunfernecia()
{
	if (!this->validacion->CeldasVacias(this->primeroA, this->auxA))
	{
		if (!this->validacion->ValidarFormato(this->primeroA, this->auxA))
		{
			...
			r = abs(this->primeroA->sig->sig->valor);
			char ABS[MAXCHAR];
			ZeroMemory(ABS, sizeof(ABS));
			sprintf_s(ABS, "%f", r);
			SetWindowText(this->primeroA->sig->sig->celda, ABS);
			x = centroX;
			y = r;
			...
			while (x < y)
			{
				...
				if (p < 0)
				{
					...
					sprintf_s(A, "%f", 2.0f * x);
					sprintf_s(B, "%f", 2.0f * y);
					sprintf_s(PK, "%f", p);
...
void Bresenham::Elipse()
{
	if (!this->validacion->CeldasVacias(this->primeroA, this->auxA))
	{
		if (!this->validacion->ValidarFormato(this->primeroA, this->auxA))
		{
			...
			rx = abs(this->primeroA->valor);
			ry = abs(this->primeroA->sig->valor);
			...
			ZeroMemory(ABS, sizeof(ABS));
			sprintf_s(ABS, "%f", rx);
			SetWindowText(this->primeroA->celda, ABS);

			ZeroMemory(ABS, sizeof(ABS));
			sprintf_s(ABS, "%f", ry);
			SetWindowText(this->primeroA->sig->celda, ABS);
			...
			int contador = 0;
			while ((2.0f * pow(ry, 2) * x) < (2.0f * pow(rx, 2) * y))
			{
				if (p1 < 0.0f)
				{
					x = x + 1.0f;
					...
					sprintf_s(X, "%f", x);
					sprintf_s(Y, "%f", y);
					sprintf_s(CONDICIONAL_A, "%f", ((2.0f * pow(ry, 2)) * (x)));
					sprintf_s(CONDICIONAL_B, "%f", ((2.0f * pow(rx, 2)) * (y)));
					...
					SendMessage(this->editResultado, LB_INSERTSTRING, contador, (LPARAM)Punto);
					contador++;
					p1 = p1 + ((2.0f) * (pow(ry, 2)) * (x)) + pow(ry, 2);
				}
				else
				{
					x = x + 1.0f;
					...
					SendMessage(this->editResultado, LB_INSERTSTRING, contador, (LPARAM)Punto);
					contador++;
					p1 = p1 + ((2.0f) * (pow(ry, 2)) * (x)) - ((2.0f) * (pow(rx, 2)) * (y)) + pow(ry, 2);
				}
			}

			p2 = pow(ry, 2);
			p2 = p2 * pow(((x)+(1.0f / 2.0f)), 2);
			p2 = p2 + (pow(rx, 2) * pow((y - 1.0f), 2));
			...
			while (yp != 0.0f)
			{
				if (p2 > 0.0f)
				{
					...
					SendMessage(this->editResultado, LB_INSERTSTRING, contador, (LPARAM)Punto);
					contador++;
					p2 = p2 - ((2.0f) * (pow(rx, 2)) * (y)) + pow(rx, 2);
				}
				else
				{
					x = x + 1.0f;
					...
					SendMessage(this->editResultado, LB_INSERTSTRING, contador, (LPARAM)Punto);
					contador++;
					p2 = p2 + ((2.0f) * (pow(ry, 2)) * (x)) - ((2.0f) * (pow(rx, 2)) * (y)) + pow(rx, 2);
				}
			}
			this->MostrarBotonResultado();
			this->DeshabilitarBotones();
